  ...
  describe Options do
    ...
    describe 'parsing' do
      ...
      context '-p PROFILE or --profile PROFILE' do
        ...
        it "does not includes STDOUT formatters from the profile if there is a STDOUT formatter in command line" do
          given_cucumber_yml_defined_as({'html' => %w[--format html -o features.html --format progress]})
          options.parse!(%w{--format progress --profile html})
          options[:formats].should == [['progress', output_stream], ['html', 'features.html']]
        end
